...
import { u as _e, p as ze, G as D, H as Ee, I as Te, J as Pe, K as Oe, N as b, O as G, P as We, M as ne, Q as $e, R as Be, U as Le, V as A, W as De, X as Ge, Y as Fe, Z as Ue, _ as He, E as Ke, o as Ve, T as qe } from "./main-m0GtFhHC.js";
...
const Je = { ShieldCheck: s.jsx(G, { size: 12 }), Search: s.jsx(He, { size: 12 }), Database: s.jsx(Ue, { size: 12 }), Newspaper: s.jsx(Fe, { size: 12 }) };
function Qe({ session: i, isActive: t, onClick: k, onRename: v, onDelete: R }) {
  ...
  }, className: b("p-1 rounded-md transition-opacity", t ? "hover:bg-white text-gray-400" : "hover:bg-white/60 text-gray-300", w ? "opacity-100" : "opacity-0 group-hover:opacity-100"), children: s.jsx(Ke, { size: 11 }) }), w && s.jsxs(s.Fragment, { children: [s.jsx("div", { className: "fixed inset-0 z-10", onClick: () => d(false) }), s.jsxs("div", { className: "absolute right-0 top-full mt-1 bg-white rounded-xl border border-gray-100 shadow-xl z-20 min-w-[120px] py-1", children: [s.jsxs("button", { onClick: (u) => {
    u.stopPropagation(), v(), d(false);
  }, className: "flex items-center gap-2 w-full px-3 py-2 text-xs text-gray-700 hover:bg-gray-50", children: [s.jsx(Ve, { size: 11 }), " \u91CD\u547D\u540D"] }), s.jsxs("button", { onClick: (u) => {
    u.stopPropagation(), R(), d(false);
  }, className: "flex items-center gap-2 w-full px-3 py-2 text-xs text-danger-600 hover:bg-danger-50", children: [s.jsx(qe, { size: 11 }), " \u5220\u9664"] })] })] })] })] });
}
function st() {
  var _a;
  const i = _e(), { sessionId: t } = ze(), { sessions: k, messages: v, intelligenceSkills: R, messagePagination: w, isStreaming: d, messagesLoadingBySession: u, wsStatus: h, wsReconnectAttempt: oe, wsReconnectMaxAttempts: le, error: ie, fetchSessions: p, fetchMessagesBySession: S, setActiveMessageSession: I, clearSessionMessages: _, loadOlderMessages: ce, createSession: F, updateSession: de, deleteSession: ue, sendMessageWS: U, uploadResource: fe, connectWebSocket: z, disconnectWebSocket: H, retryWebSocketConnection: E, abortActiveMessageStream: K, fetchResources: V, fetchIntelligenceSkills: q, executeIntelligenceSkill: xe } = D(), { addToast: m } = Ee(), { confirm: he, prompt: pe } = Te(), J = !!((_a = Pe()) == null ? void 0 : _a.leftCollapsed), [y, Q] = n.useState(null), [me, X] = n.useState(false), [c, ge] = n.useState([]), [j, T] = n.useState(null), P = n.useRef(0), O = n.useRef(void 0), Y = n.useRef({}), Z = n.useRef({});
  n.useEffect(() => {
    p(), q(), Oe.listGroups().then(ge).catch(() => {
    });
  }, [p, q]), n.useEffect(() => {
    if (c.length === 0) {
      T(null);
  ...
  const be = async () => {
    ...
    if (!a) return;
    const r = await pe({ title: "\u91CD\u547D\u540D\u4F1A\u8BDD", message: "\u8BF7\u8F93\u5165\u65B0\u7684\u4F1A\u8BDD\u540D\u79F0", defaultValue: a.title, placeholder: "\u4F1A\u8BDD\u540D\u79F0" });
    ...
    let B = [];
    if (C.length > 0) try {
      B = await Le(`upload ${C.length} local file(s)`, { sessionId: M, files: C.map((o) => o.name) }, async () => {
        const o = [];
        for (const x of C) {
          const L = x.file;
          A("upload_start", "uploading local file before send", { tempId: x.id, fileName: L.name, destination: "POST /api/sessions/:id/upload \u2192 AI SDK cloud + DB resource row" });
          const f = await fe(M, L);
          await De(f.id, L, Ge(f.id, f.url).filter((Ie) => Ie !== f.id)), o.push({ id: f.id, name: f.name || x.name, type: f.type || "file" }), A("upload_success", "mapped temp attachment to server resource id", { tempId: x.id, resourceId: f.id, url: f.url });
        }
        return o;
      });
    } catch (o) {
      const x = o instanceof Error ? o.message : "\u6587\u4EF6\u4E0A\u4F20\u5931\u8D25";
      A("upload_failure", x, { sessionId: M }, "error"), m("error", x);
      return;
    }
    const ae = [...se, ...B];
    A("send_message", "sending via WebSocket with resource_refs", { sessionId: M, resourceRefs: ae, localUploaded: B.length, libraryAttached: se.length }), U(M, e, ae);
  }, Se = (e) => {
    if (d) {
    ...
    if (y) try {
      ...
      U(r, a, []), m("success", `${y.name} \u5DF2\u63D0\u4EA4`);
    } catch (a) {
      m("error", (a == null ? void 0 : a.message) || "\u63D0\u4EA4\u5931\u8D25");
    }
  }, N = n.useMemo(() => c.find((e) => e.id === j) ?? c[0] ?? null, [c, j]), W = n.useMemo(() => {
