...
import { q as de, a as Se, R as ve, U as ke, L as b, W as le, M as Ee, X as $e, Y as Ce, c as Le, Z as _e } from "./main-DXuYf9zK.js";
import { p as ze } from "./themePresets-JHfH1TGp.js";
import { o as f, a as ue, i as W } from "./routes-ClgxGlHZ.js";
import { S as Me, P as Ie } from "./PptxgenjsEditor-CI1E2BDR.js";
import "./charts-S2HjfYzV.js";
import "./routes-UgFNxPGD.js";
import "./pptx-worker-Co20o6hA.js";
import "./PipelineProgressPanel-BzwUNZLj.js";
const S = /* @__PURE__ */ new Map(), Re = 8;
function Te(s) {
  return s.pageId || s.id || `page-${s.pageNumber}`;
}
function Ae(s, c, l, i) {
  const m = c.map((p) => `${Te(p)}:${p.pageNumber}:${p.status ?? ""}:${p.title}`).join("|");
...
function Ze() {
  const { projectId: s } = de(), c = Se(), l = r.useRef(false), [i, m] = r.useState(""), [p, v] = r.useState(0), [k, pe] = r.useState(""), [u, q] = r.useState([]), [Fe, j] = r.useState(""), [ge, E] = r.useState(""), [Ue, Y] = r.useState([]), [L, N] = r.useState(false), [J, g] = r.useState(""), [R, O] = r.useState(false), [T, Q] = r.useState(false), [xe, Ge] = r.useState(0), [A, B] = r.useState(""), [D, V] = r.useState(false), [H, h] = r.useState([]), [_, ee] = r.useState(""), [w, fe] = r.useState("pptx"), [me, X] = r.useState(null), [he, te] = r.useState(false), ye = r.useRef(null), Z = r.useRef(false), se = r.useMemo(() => u.filter((e) => e.status === "completed" || e.status === "generated").length, [u]), be = u.length, z = r.useMemo(() => !s || u.length === 0 ? "" : Ae(s, u, p, {}), [s, u, p]), P = r.useCallback(async () => {
    if (!s) return null;
    ...
    n && pe(n);
    let o = e.messages;
    return (o == null ? void 0 : o.length) || (o = await f.getMessages(s)), h(Xe(o || [], n)), e;
  }, [s]), x = r.useCallback((e) => {
    Y((a) => [...a.slice(-40), e]);
    ...
    if ((e.type === "stage_started" || e.type === "stage_progress") && (j(e.payload.stage), e.payload.label && E(e.payload.label), x(e.payload.label || e.payload.stage)), e.type === "llm_status" && e.payload.detail && x(e.payload.detail), e.type === "assistant_message" && (x(e.payload.content.slice(0, 200)), h((a) => [...a, { role: "assistant", content: e.payload.content }])), e.type === "page_generated" || e.type === "page_updated") {
      ...
      }), x(`\u7B2C ${a.pageNumber} \u9875\uFF1A${a.title}`), F(e.payload.stage, `
\u7B2C ${a.pageNumber} \u9875\uFF1A${a.title}`);
    }
    e.type === "page_started" && (x(`\u751F\u6210\u7B2C ${e.payload.pageNumber} \u9875\u2026`), ae(e.payload.stage, oe(e.payload.stage), `\u751F\u6210\u7B2C ${e.payload.pageNumber} \u9875\u2026`)), e.type === "page_failed" && (x(`\u7B2C ${e.payload.pageNumber} \u9875\u5931\u8D25\uFF1A${e.payload.error || ""}`), F(e.payload.stage, `
\u7B2C ${e.payload.pageNumber} \u9875\u5931\u8D25`)), e.type === "run_completed" && (j("run_completed"), E("\u751F\u6210\u5B8C\u6210"), x("\u5168\u90E8\u5B8C\u6210"), U()), e.type === "run_error" && (j("run_error"), g(e.payload.message), x(e.payload.message), U());
  }, [x, ae, F, U]), G = r.useCallback(async () => {
    if (!(!s || L)) {
      N(true), g(""), Y([]), j("planning"), E("\u51C6\u5907\u751F\u6210\u2026");
      try {
        await ce(s, $), await P();
      } catch (e) {
  ...
  r.useEffect(() => {
    ...
    return (async () => {
      try {
        const a = await P();
        if (e || !(a == null ? void 0 : a.activeRun) || a.activeRun.status !== "running" || Z.current || l.current) return;
        Z.current = true, N(true), j("planning"), E("\u6B63\u5728\u751F\u6210\u2026");
        try {
    ...
    var _a;
    ((_a = c.state) == null ? void 0 : _a.autoRun) && !l.current && s && (l.current = true, G());
  }, [c.state, s, G]);
  const re = u[xe], K = re ? ie(re) : "";
  r.useEffect(() => {
    if (!s || !K) {
      B("");
      return;
    ...
    let e = false;
    return V(true), f.getPageHtml(s, K).then((a) => {
      e || B(a);
    }).catch(() => {
      e || B("");
    }).finally(() => {
...
function st() {
  ...
  }, [s, l]), p ? t.jsxs("div", { className: "flex h-full min-h-0 items-center justify-center bg-gray-50 text-sm text-gray-500", children: [t.jsx(b, { className: "mr-2 animate-spin", size: 18 }), "\u52A0\u8F7D\u9879\u76EE\u2026"] }) : i === "pptxgenjs" && s ? t.jsx(Ie, { homePath: ue(), useSlideGlancePreview: true }) : t.jsx(Ze, {});
}
export {
  st as default
};
