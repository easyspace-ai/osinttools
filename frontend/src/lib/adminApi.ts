...
import { AuthRequiredError, ensureAuthenticatedSession } from "@/osint/auth";
...
export async function listUsers(): Promise<AdminUser[]> {
  const res = await fetch("/api/admin/users", authFetchInit({ headers: authHeaders() }));
...
export async function createUser(payload: {
  ...
  const res = await fetch("/api/admin/users", authFetchInit({
    method: "POST",
...
export async function updateUser(
  ...
  const res = await fetch(`/api/admin/users/${id}`, authFetchInit({
    method: "PATCH",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }));
...
export async function toggleDisableUser(id: string): Promise<AdminUser> {
  const res = await fetch(`/api/admin/users/${id}/disable`, authFetchInit({
    method: "PATCH",
    headers: authHeaders(),
  }));
...
export async function resetPassword(id: string, newPassword: string): Promise<{ password: string }> {
  const res = await fetch(`/api/admin/users/${id}/reset-password`, authFetchInit({
    method: "POST",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
...
export async function listRoles(): Promise<RoleItem[]> {
  const res = await fetch("/api/admin/roles", authFetchInit({ headers: authHeaders() }));
...
export async function createRole(payload: { name: string; description?: string; permissions: string[]; skill_group_id?: string }) {
  const res = await fetch("/api/admin/roles", authFetchInit({
    method: "POST",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }));
...
export async function updateRole(id: string, payload: { name: string; description?: string; permissions: string[]; skill_group_id?: string }) {
  const res = await fetch(`/api/admin/roles/${id}`, authFetchInit({
    method: "PUT",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }));
...
export async function listSkillGroups(): Promise<SkillGroupItem[]> {
  const res = await fetch("/api/admin/skill-groups", authFetchInit({ headers: authHeaders() }));
...
export async function createSkillGroup(payload: { name: string; description?: string; skill_ids: string[]; role_id?: string }) {
  const res = await fetch("/api/admin/skill-groups", authFetchInit({
    method: "POST",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }));
...
export async function updateSkillGroup(id: string, payload: { name: string; description?: string; skill_ids: string[]; role_id?: string }) {
  const res = await fetch(`/api/admin/skill-groups/${id}`, authFetchInit({
    method: "PUT",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }));
...
export async function fetchAdminSettings(): Promise<AdminSettings> {
  const res = await fetch("/api/admin/settings", authFetchInit({ headers: authHeaders() }));
...
export async function updateAdminSettings(payload: Partial<AdminSettings>): Promise<AdminSettings> {
  const res = await fetch("/api/admin/settings", authFetchInit({
    method: "PATCH",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }));
...
/** 轮询类请求：复用 HttpOnly Cookie，避免每次打 /auth/me 触发限流。 */
async function xstreamAdminFetch(
  url: string,
  init: RequestInit = {},
): Promise<Response> {
  const res = await fetch(url, authFetchInit(init));
  if (res.status === 401) {
