...
import {
  Plus, MessageSquare, ShieldCheck, Search, Database, Newspaper,
...
export default function IntelligenceHome() {
  ...
  const toolbarSkills = useMemo(() => {
    const enabledSkills = intelligenceSkills.filter((s) => s.is_enabled)
    const skillMap = new Map(enabledSkills.map((s) => [s.key, s]))

    if (!activeGroup) {
      return skillGroups.length === 0 ? enabledSkills : []
    }

    const ordered: IntelligenceSkill[] = []
    for (const key of activeGroup.skill_ids) {
      const s = skillMap.get(key)
      if (s) ordered.push(s)
    }
    return ordered
  }, [intelligenceSkills, activeGroup, skillGroups.length])

  // Skill 快捷工具条：按技能组 Tab 切换，组内技能平铺展示
  ...
  }, [skillGroups, activeGroupId, activeGroup, toolbarSkills, isStreaming])
